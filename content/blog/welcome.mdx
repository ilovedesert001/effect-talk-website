...
- **GitHub authentication**. Continue with your GitHub account via [WorkOS](https://workos.com). Sign-in is for saved progress and API keys — not for a mailing list.
...
Because documentation tells you what the API _does_. When you're staring at a production codebase and need to figure out how to compose three services that each need their own error channel, retry policy, and resource lifecycle — the docs give you the primitives. What you actually need is a worked example from someone who has done it before. That's the difference between a reference and a curriculum.
