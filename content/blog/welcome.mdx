...
Effect does the same thing for TypeScript, but the organizing concept is the _effect_. An HTTP request is an effect. A database query is an effect. Reading a config file, logging a message, retrying a failed operation — all effects. They compose the same way. They're handled the same way. They get tested the same way.
...
- **GitHub authentication**. Sign in with GitHub via [WorkOS](https://workos.com). Sign-in is for saved progress and API keys — not for a mailing list.
